from X_src.model.diffusion_lib.nlm import LogicMachine
from X_src.model.diffusion_lib.transformer import GPT
import einops
...
class SudokuLatentEBM(nn.Module):
    ...
    def forward(self, x, t):
        ...
        return energy
class MazeEBM(nn.Module):
    def __init__(self, inp_dim=1, out_dim=1):
        super().__init__()
        ...
        h = 384
        self.conv1 = nn.Conv2d(7, h, 3, padding=1)
    ...
    def forward(self, x, t):
        ...
        '''
        t_emb = self.time_mlp(t)
        x = x.permute(0, 3,1,2) # [B,h,w,c] -> [B,c,h,w],c = c_wall+c_goal_c_path = 2 +3+2 = 7
...
class MazeDenoise(nn.Module):
    def __init__(self, inp_dim=7, out_dim=2):
        super().__init__()
        """
        Modified Denoising Network for Maze Generation
        Args:
            inp_dim: Channel dimension of input conditions (default:7)
            out_dim: Channel dimension of output variables (default:2)
        """
        ...
        # Modification 1: Input channels adjustment
        h = 384  # Base channel size
        self.conv1 = nn.Conv2d(inp_dim + out_dim, h, 3, padding=1)

        # Residual blocks with time conditioning
        self.res1a = ResBlock(downsample=False, rescale=False, filters=h, time_dim=time_dim)
        self.res1b = ResBlock(downsample=False, rescale=False, filters=h, time_dim=time_dim)
        
        # Optional attention layers (commented by default)
        self.attn1 = Attention(h, dim_head=128)
        ...
        # Modification 2: Output layer adjustment
        self.conv5 = nn.Conv2d(h, out_dim, 1, padding=0)

        self.inp_dim = inp_dim
        self.out_dim = out_dim

    def forward(self, inp, xt, t,return_energy=False, return_both=False):
        """
        Forward pass with shape preservation
        Args:
            inp: [B, H, W, inp_dim]  Input conditions
            xt: [B, H, W, out_dim]   Noisy variables to denoise
            t: [B,]                  Diffusion timesteps
        Returns:
            output: [B, H, W, out_dim] Denoised output with same shape as xt
        """
        # Modification 3: Channel-wise concatenation
        x = torch.cat([inp, xt], dim=-1)  # [B, H, W, inp_dim+out_dim]
        
        # Dimension transformation for Conv2d
        ...
        h = self.res1a(h, t_emb)
        h = self.res1b(h, t_emb)
        # h = self.attn1(h)  # Optional attention
        ...
        # Final projection
        output = self.conv5(h)  # [B, out_dim, H, W]
        
        # Restore original dimension order
        output = output.permute(0, 2, 3, 1)  # [B, H, W, out_dim]
        if return_energy:
            energy = torch.zeros(output.shape[0],1).to(output.device)
            return energy
        elif return_both:
            energy = torch.zeros(output.shape[0],1).to(output.device)
