...
from X_src.model.diffusion_lib.nlm_utils import Expander, Reducer, Permutation, LogicInference
...
class LogicLayer(nn.Module):
    """
        Logic Layers do one-step differentiable logic deduction. The predicates
        grouped by their number of variables. The inter group deduction is done
        by expansion/reduction, the intra group deduction is done by logic model.
    """
    def __init__(
        ...
        """
            Other arguments see help information in make_prog_block_parser.

            Args:
                input_dims: the number of input channels of each input group,
                            should consist with the inputs. use dims=0 and
                            input=None to indicate no input of that group.
        ...
        if breadth > 3:
            warnings.warn('Using LogicLayer with breadth > 3 may cause speed and memory issue.')
        ...
        for i in range(self.max_order + 1):
            ...
            if current_dim == 0:
                ...
                self.logic.append(LogicInference(current_dim, output_dims[i], logic_hidden_dim))
    ...
    __hyperparams__ = (
        'breadth', 'input_dims', 'output_dims', 'logic_hidden_dim',
...
class LogicMachine(nn.Module):
    """
        Neural Logic Machine consists of logic layers, and its forward propagation
        performs differentiable logic deduction, its backward propagation mimics
        logic induction.
    """
    def __init__(
            self, depth, breadth, input_dims, output_dims,
            logic_hidden_dim, exclude_self=True,
            residual=False, io_residual=False, recursion=False, connections=None,
    ):
        """
            Other arguments see help information in make_prog_block_parser.

            Args:
                input_dims: the number of input channels of each input group for
                            the first layer, should consist with the inputs. use
    ...
    __hyperparams__ = (
        'depth', 'breadth', 'input_dims', 'output_dims', 'logic_hidden_dim',
        'exclude_self', 'io_residual', 'residual', 'recursion',
    )
    ...
    def make_prog_block_parser(cls, parser, defaults, prefix=None):
        ...
        parser.add_argument(prefix + 'exclude-self', type='bool',
                            default=defaults['exclude_self'], metavar='B',
                            help='not allow multiple occurrence of same variable')
        parser.add_argument(prefix + 'io-residual', type='bool',
                            default=defaults['io_residual'], metavar='B',
                            help='use input/output-only residual connections')
        parser.add_argument(prefix + 'residual', type='bool',
                            default=defaults['residual'], metavar='B',
                            help='use residual connections')
        parser.add_argument(prefix + 'recursion', type='bool',
                            default=defaults['recursion'], metavar='B',
